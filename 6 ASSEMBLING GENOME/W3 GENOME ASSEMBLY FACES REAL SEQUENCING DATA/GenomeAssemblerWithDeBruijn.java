...
public class GenomeAssemblerWithDeBruijn {

    private static final String FILES_FOLDER = "Coursera_algorithms_and_data_structures_specialization\\6_assembling-genomes\\week3\\files\\";
        ...
        new Thread(null, new Runnable() {
            public void run() {
                try (
                    ...
                    // PrintStream out = new PrintStream(new File(FILES_FOLDER + "log.log"));
                    // Scanner scanner = new Scanner(new BufferedReader(new FileReader(new File(FILES_FOLDER + "dataset.txt"))))
                    ) {
                    OUT = out;
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        paired = paired || read.contains("|");
                        if(paired) {
                            String[] splitted = read.split("|");
                            helper.createKmersFromRead(splitted[0]);
                            helper.createKmersFromRead(splitted[1]);
                        } else
    ...
    static class DeBruijnGraphHelper {
        ...
        private void correctKmers() {
            ...
            List<String> keys = kmersOccurrenciesMap.entrySet().stream()
                    .filter(e -> e.getValue() <= averageOccurrency / 2).map(e -> e.getKey())
        ...
        public boolean makeGraphBalanced() throws Exception {
            if (DEBUG)
        ...
        private void fix(Set<Vertex> startVertexes, Set<Vertex> endVertexes, Set<Vertex> unbalancedInVertexes,
                Set<Vertex> unbalancedOutVertexes) throws Exception {
            ...
            if (K < graph.size()) {
                ...
                if(DEBUG)
                    OUT.println("Stop increasing K, trying to assemble all disjoined graphs");
                int index = 1;
                for (Vertex v : startVertexes) {
                    ...
                    while (!stack.isEmpty()) {
                        ...
                        outs.put(current.id, current.outEdges.size());
                        for (Edge edge : current.outEdges)
                            stack.add(graph.get(edge.to));
                    }
                    dfs(v, cycle, outs);
                    if (cycle.size() > 100) {
                        OUT.println("\n>CONTIG" + index++);
        ...
        private Map<Integer, List<List<Vertex>>> reconstructPaths(DFSResult result, List<Integer> errors) {
            ...
            for (Integer error : errors) {
                ...
                for (Integer parent : result.parents.get(error)) {
                    ...
                    continuePath(result.startingPoint, paths.get(error), startingPath, new HashMap<>(result.parents));
                }
            }
            return paths;
        }
        ...
        private void continuePath(Integer searchVertex, List<List<Vertex>> bubblePaths, List<Vertex> currentPath,
            ...
            parents.remove(currentPath.get(currentPath.size() - 1).id);
